...
export default function KineticGrid({
    ...
    trailColor = "rgba(212,175,55,0.4)",
    spacing = 40,
    radius = 180,
    strength = 4,
    trail = true,
    ...
    useEffect(() => {
        ...
        const frame = () => {
            ...
            // Calculate speed and dynamically adjust attraction radius & strength
            let currentRadius = R;
            let currentPull = PULL;
            if (m.active && prevMouseRef.current.x !== -9999) {
                const mx = m.x - prevMouseRef.current.x;
                const my = m.y - prevMouseRef.current.y;
                const speed = Math.min(30, Math.sqrt(mx * mx + my * my));
                currentRadius = R + speed * 3.5;
                currentPull = PULL * (1 + speed * 0.08);
            }
            prevMouseRef.current.x = m.x;
            prevMouseRef.current.y = m.y;
            ...
            for (const d of dots) {
                ...
                if (m.active) {
                    ...
                    if (dist < currentRadius && dist > 0.001) {
                        const f = (1 - dist / currentRadius) * currentPull;
                        ax += (dx / dist) * f;
                        ay += (dy / dist) * f;
                    }
                }
                // Damping adjusted to 0.88 for more springy organic jiggle!
                d.vx = (d.vx + ax) * 0.88;
                d.vy = (d.vy + ay) * 0.88;
                d.x += d.vx;
                d.y += d.vy;
            ...
            for (let c = 0; c < cols.length; c++) {
                for (let rIdx = 0; rIdx < cols[c].length; rIdx++) {
                    ...
                    const prox = m.active
                        ...
                        : 0;
                    if (right) {
                        // Base opacity increased to 0.16 to show faint golden grid at all times
                        ctx.globalAlpha = 0.16 + prox * 0.72;
                        ctx.strokeStyle = lineColor;
            ...
            for (const d of dots) {
                const prox = m.active
                    ? Math.max(
                          0,
                          1 - Math.sqrt((m.x - d.x) ** 2 + (m.y - d.y) ** 2) / currentRadius
                      )
                    : 0;
                // Base opacity of dots increased to 0.35
